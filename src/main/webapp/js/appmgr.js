...
scnjs.drawUserList = function(items, sortIdx, sortOrder) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var dispUid = cInd + '<span class="pseudo-link link-button" style="text-align:center;" onclick="scnjs.editUser(\'' + username + '\');" data-tooltip2="Edit">' + username + '</span>';
    var dispFullname = scnjs.buildCopyableLabel(fullname);
    var dispLocalFullname = scnjs.buildCopyableLabel(localfullname);
    ...
    if (loginFailedCount > 0) {
      ...
      htmlList += '<span class="' + clz + '" data-tooltip="Last failed: ' + loginFailedTime + '" onclick="scnjs.confirmClearLoginFailedCount(\'' + username + '\');">' + loginFailedCount + '</span>';
    } else {
      htmlList += '';
    }
    htmlList += '</td>';
