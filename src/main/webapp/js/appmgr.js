...
app.appmgr.getUserInfoListCb = function(xhr, res) {
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    var dt = app.appmgr.elapsedSinceLastAccess(now, lastAccessDate);
    user.elapsed = dt;
    userList.push(user);
  }
...
app.appmgr.drawUserList = function(items, sortIdx, sortOrder) {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var lastAccessDate = app.appmgr.getDateTimeString(statusInfo.last_access, app.appmgr.INSEC);
    ...
    var led = app.appmgr.buildLedHtml(now, statusInfo.last_access, app.appmgr.INSEC, active);
